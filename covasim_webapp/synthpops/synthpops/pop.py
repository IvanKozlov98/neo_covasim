...
def round_dict_values(input_dict, precision=3):
    # Создаем новый словарь для хранения округленных значений
    rounded_dict = {}

    # Проходим по каждому ключу и значению в исходном словаре
    for key, value in input_dict.items():
        # Если значение - это массив (numpy array), округляем его
        rounded_value = np.round(value, precision)
        # Добавляем округленное значение в новый словарь
        rounded_dict[key] = rounded_value
        print(f"Rounded {key}")
        for vv in rounded_value:
            for ff in vv:
                print('{0:.3f}'.format(ff), end=' ')
            print()
...
class Pop(sc.prettyobj):
    ...
    def generate(self):
        ...
        if self.contact_matrices is None:
            contact_matrices = spdata.get_contact_matrices(datadir, file_path_dic=self.file_path_dic, sheet_name=sheet_name)
