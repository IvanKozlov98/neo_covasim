...
@nb.njit(             (nbfloat,  nbint[:],  nbint[:], nbfloat[:],   nbfloat[:], nbfloat[:], nbfloat[:], nbbool), cache=cache, parallel=rand_parallel)
def compute_infections_version_2(beta,     p1,        p2,       layer_betas,  rel_trans,  rel_sus,    cur_betas, legacy=False): # pragma: no cover
    '''
    Compute who infects whom
    ...
    for sources,targets in pairs:
        ...
        nonzero_sources  = sources[nonzero_inf_inds] # Remove zero entries from the sources
        nonzero_targets  = targets[nonzero_inf_inds] # Remove zero entries from the targets
        new_betas[nonzero_targets] = new_betas[nonzero_targets] + nonzero_betas
        transmissions    = (new_betas[nonzero_targets] > 0.03).nonzero()[0] # Compute the actual infections!
        source_inds      = nonzero_sources[transmissions]
        target_inds      = nonzero_targets[transmissions] # Filter the targets on the actual infections
        slist = np.concatenate((slist, source_inds), axis=0)
        tlist = np.concatenate((tlist, target_inds), axis=0)
    return slist, tlist, new_betas
...
def sample(dist=None, par1=None, par2=None, size=None, **kwargs):
    ...
    - 'beta'          : binom distribution with a=par1 and b=par2
    ...
    **Examples**::
    ...
    elif dist == 'beta':              samples = np.random.beta(a=par1, b=par2, size=size) * kwargs['step'] # Use custom version below
    elif dist == 'special_test_delay': samples = np.random.choice(15, size, replace=True, p=[
                                                                                            0.007979865,
                                                                                            0.1368106,
                                                                                            0.224073842,
                                                                                            0.120563417,
