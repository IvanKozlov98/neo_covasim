...
def single_run(sim, ind=0, reseed=True, noise=0.0, noisepar=None, keep_people=False, run_args=None, sim_args=None, verbose=None, do_run=True, barrier=None, lock=None, return_list=None, shared_tourists=None, shared_back=None, adjacency_matrix=None, tourist_fraction=None, invariant_var=None, **kwargs):
    '''
    ...
    **Example**::
    ...
    if do_run:
        if barrier is not None:
            run_args = sc.mergedicts({'barrier': barrier, 'lock': lock, 'sims_own_ind': ind, 'shared_tourists': shared_tourists, 'shared_back': shared_back, 'adjacency_matrix': adjacency_matrix, 'invariant_var': invariant_var, 'tourist_fraction': tourist_fraction}, run_args)
        sim.run(**run_args)
...
def mulitple_cities_procedure(single_run_func, sims, args_for_sims, adjacency_matrix, tourist_fraction):
    sim_count = len(sims.get('sim'))
    processes = []
    processes_count = sim_count
    barrier = multiprocessing.Barrier(processes_count)
    lock = multiprocessing.Lock()

    # define max tourists count
    max_tourist_count = np.zeros(adjacency_matrix.shape[0], dtype=np.int64)
    for from_city_ind in range(adjacency_matrix.shape[0]):
        for to_city_ind in range(adjacency_matrix.shape[0]):
            if from_city_ind == to_city_ind:
                continue
            max_tourist_count[to_city_ind] += (14 * sims.get('sim')[from_city_ind].pars['pop_size'] * adjacency_matrix[from_city_ind, to_city_ind])
    #
    tourist_fractions = []
    for city_ind in range(adjacency_matrix.shape[0]):
        tourist_fractions.append(max(max_tourist_count[city_ind] / sims.get('sim')[city_ind].pars['pop_size'], 0.1))
    ...
    kw['return_list'] = return_list

    kw['shared_back'] = manager.list([manager.list(processes_count * [None]) for _ in range(processes_count)])
    kw['shared_tourists'] = manager.list([manager.list(processes_count * [None]) for _ in range(processes_count)])
    kw['adjacency_matrix'] = adjacency_matrix
    kw['tourist_fraction'] = manager.list(tourist_fractions)
    kw['invariant_var'] = manager.Value('i', 0)

    for i in range(processes_count):
        kw['sim'] = sims.get('sim')[i]
        kw['ind'] = sims.get('ind')[i]
        processes.append(multiprocessing.Process(target=single_run_func, kwargs=kw))
...
def multi_run(sim, n_runs=4, reseed=None, noise=0.0, noisepar=None, iterpars=None,
              ...
              do_run=True, parallel=True, n_cpus=None, verbose=None, retry='warn', mulitple_cities=False, adjacency_matrix=None, tourist_fraction=None, **kwargs):
    '''
    ...
    **Example**::
    ...
    if parallel:
        ...
        try:
            if not mulitple_cities:
                sims = sc.parallelize(single_run, **kw) # Run in parallel
            else:
                sims = mulitple_cities_procedure(single_run, iterkwargs, kwargs, adjacency_matrix, tourist_fraction)
