...
class clip_edges(Intervention):
    ...
    **Examples**::
    ...
    def __init__(self, days, changes, layers=None, func_people_isolated_inds=None, **kwargs):
        super().__init__(**kwargs) # Initialize the Intervention object
        self.days     = sc.dcp(days)
        self.changes  = sc.dcp(changes)
        self.layers   = sc.dcp(layers)
        self.contacts = None
        self.func_people_isolated_inds = func_people_isolated_inds
        self.people_isolated_inds = None
        self.n_sim_selected = None
        self.cur_sim_selected = None
        self.arange_arr = None
        return
    ...
    def initialize(self, sim):
        ...
        if self.func_people_isolated_inds is not None:
            self.people_isolated_inds = self.func_people_isolated_inds(sim)
            self.people_isolated_flags = np.zeros(shape=len(sim.people), dtype=bool)
            self.people_isolated_flags[self.people_isolated_inds] = True
        return


    def apply(self, sim):

        def total_apply_impl(ind, lkey):
        ...
        def selected_apply_impl(ind, lkey):
            s_layer = sim.people.contacts[lkey] # Contact layer in the sim
            i_layer = self.contacts[lkey] # Contact layer in the intervention
            n_sim = np.count_nonzero(self.people_isolated_flags[s_layer['p1']] + self.people_isolated_flags[s_layer['p2']])
            n_int = np.count_nonzero(self.people_isolated_flags[i_layer['p1']] + self.people_isolated_flags[i_layer['p2']])
            n_contacts = n_sim + n_int # Total number of contacts
            if n_contacts:
                current_prop = n_sim/n_contacts # Current proportion of contacts in the sim, e.g. 1.0 initially
                desired_prop = self.changes[ind] # Desired proportion, e.g. 0.5
                ...
                from_sim = (n_to_move>0) # Check if we're moving contacts from the sim
                if from_sim: # We're moving from the sim to the intervention
                    contact_inds = cvu.choose(max_n=n_sim, n=n_to_move)
                    tmp = self.people_isolated_flags[s_layer['p1']] + self.people_isolated_flags[s_layer['p2']]
                    inds = (self.people_isolated_flags[s_layer['p1']] + self.people_isolated_flags[s_layer['p2']]).nonzero()[0][contact_inds]
                    to_move = s_layer.pop_inds(inds)
                    i_layer.append(to_move)
                else: # We're moving from the intervention back to the sim
                    contact_inds = cvu.choose(max_n=n_int, n=abs(n_to_move))
                    inds = (self.people_isolated_flags[i_layer['p1']] + self.people_isolated_flags[i_layer['p2']]).nonzero()[0][contact_inds]
                    to_move = i_layer.pop_inds(inds)
                    s_layer.append(to_move)
            else: # pragma: no cover
                warnmsg = f'Warning: clip_edges() was applied to layer "{lkey}", but no edges were found; please check sim.people.contacts["{lkey}"]'
        ...
        is_total_case = self.people_isolated_inds is None
        # If this day is found in the list, apply the intervention
        for ind in find_day(self.days, sim.t, interv=self, sim=sim):
            ...
            for lkey in self.layers:
                if is_total_case:
                    total_apply_impl(ind, lkey)
                else:
                    selected_apply_impl(ind, lkey)
        return
...
class test_num(Intervention):
    ...
    **Examples**::
    ...
    def apply(self, sim):
        ...
        test_probs = np.ones(len(sim.people.symptomatic)) # Begin by assigning equal testing weight (converted to a probability) to everyone
...
class test_prob(Intervention):
    ...
    **Examples**::
    ...
    def apply(self, sim):
        ...
        test_probs = np.zeros(len(sim.people.symptomatic)) # Begin by assigning equal testing probability to everyone
