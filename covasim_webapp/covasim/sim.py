...
from covasim.people import People
from covasim.tourist_layer import TouristLayer
...
class Sim(cvb.BaseSim):
    ...
    **Examples**::
    ...
    def init_store_seir(self):
        from . import utils as cvu
        n_days = self['n_days']
        n_days_z = n_days + 10
        self.state_statistics = np.zeros(dtype=np.uint8, shape=(n_days_z, len(self.people)))
        self.lkey_to_ind = dict({'h': 0, 's': 1, 'w': 2, 'c': 3, 'l': 4})
        is_multiple_cities_mode = 'own_ind_city' in self.people.pars
        if is_multiple_cities_mode:
            self.lkey_to_ind['tourists_layer'] = 5
        else:
            self.lkey_to_ind['a'] = 5
        self.viral_load_by_layers = np.zeros(shape=(6, n_days_z))
        self.dangerous_contacts_count_by_layers = np.zeros(shape=(6, n_days_z))
        self.susceptible_count_per_day = np.zeros(shape=(n_days_z), dtype=int)
        
        self.infected_array = np.zeros(dtype=np.uint8, shape=(len(self.people)))
        self.infected_p1_p2_array = np.zeros(dtype=np.uint8, shape=(len(self.people)))
        self.infected_people_per_day = []
    ...
    def init_people(self, popdict=None, prepared_pop=None, init_infections=False, reset=False, verbose=None, **kwargs):
        '''
        Create the people.
    ...
    def step(self):
        ...
        self.susceptible_count_per_day[t] = len(self.people.susceptible[self.people.susceptible])
        ...
        #print(f"On time {self.t} has next histogram: {np.histogram(prel_trans)}")
        ...
        for variant in range(nv):
            ...
            if ~inf_variant.any():
                continue
            betas_2 = np.zeros(len(self.people), dtype=cvd.default_float)
            for lkey, layer in contacts.items():
                ...
                for p1,p2 in pairs:
                    if self.is_additive_formula:
                        before_betas = np.array(betas_2, copy=True)
                        source_inds, target_inds, betas_2 = cvu.compute_infections_version_2(beta, p1, p2, betas, rel_trans, rel_sus, betas_2, legacy=self._legacy_trans)  # Calculate transmission!
                        self.viral_load_by_layers[self.lkey_to_ind[lkey]][self.t] = np.sum(betas_2 - before_betas)
                    else:
                        ...
                        self.viral_load_by_layers[self.lkey_to_ind[lkey]][self.t] = np.unique(target_inds).size
                    self.infected_p1_p2_array[:] = 0
                    self.infected_p1_p2_array[self.people.exposed] = 1
                    self.infected_p1_p2_array[self.people.critical] = 1
                    self.infected_p1_p2_array[self.people.severe] = 1
                    self.infected_p1_p2_array[self.people.infectious] = 1
                    self.infected_p1_p2_array[self.people.symptomatic] = 1
                    self.dangerous_contacts_count_by_layers[self.lkey_to_ind[lkey]][self.t] = np.count_nonzero(np.logical_xor(self.infected_p1_p2_array[p1], self.infected_p1_p2_array[p2]))
    ...
    def update_people_by_rest(self, rest, inds):
        for array_member in self.people.array_members:
            if array_member == 'rel_trans_init':
                continue
            if array_member != "uid":
                self.people[array_member][inds] = rest[array_member]
        for two_array_member in self.people.two_array_members:
            for dim in range(self.people[two_array_member].shape[0]):
                self.people[two_array_member][dim][inds] = rest[two_array_member][dim]
        self.people.in_city[inds] = True

    def add_back(self, back_people: People):
        ''' write people who should return in city '''
        #if self.own_ind == 0:
        #    print(np.count_nonzero(back_people.infectious), end=' ')
        self.update_people_by_rest(back_people, back_people.true_uid)



    def add_tourists(self, tourists: People):
        ''' write people who arrived in city '''
        all_tourists_inds = self.people.uid[(self.people.uid >= self.people.pop_size) * (~self.people.in_city)]
        free_tourists_inds = all_tourists_inds[:tourists.pop_size]
        self.update_people_by_rest(tourists, free_tourists_inds)
    ...
    def select_people_from_inds(self, inds):
        ...
        for array_member in self.people.array_members:
            if array_member == 'rel_trans_init':
                continue
            selected_people[array_member][:] = self.people[array_member][inds]
        for two_array_member in self.people.two_array_members:
            for dim in range(selected_people[two_array_member].shape[0]):
                selected_people[two_array_member][dim][:] = self.people[two_array_member][dim][inds]
        return selected_people

    def extract_back(self):
        ''' read (and extract) people who should return in city '''
        # индексы людей, которые должны вернуться в свой город
        list_back_people = []
        for city_ind in range(self.cities_count):
            if city_ind == self.own_ind:
                continue
            should_back_city_inds = self.people.uid[(self.people.own_city == city_ind) *
                                                    (self.people.rest_in_another_city_days == 0)]
            # конструируем туристов, которые должны уйти из нашего города в город с индексом city_ind
            back_people = self.select_people_from_inds(should_back_city_inds)
            # чистим собственный массив people
            self.people.make_not_existing(should_back_city_inds)
            # resulting step
            list_back_people.append((city_ind, back_people))
        return list_back_people


    def extract_tourists(self):
        ''' read (and extract) people who arrived in city '''
        all_people_left_city_count = cvu.poisson(float(self['pop_size']) * self.outflow_ratio)
        all_people_left_city_inds = cvu.choose(max_n=self.people.pop_size, n=all_people_left_city_count)
        # split people to cities
        people_left_city_to_city_inds = np.array_split(all_people_left_city_inds,
                                                       np.cumsum((self.outflow_ratio_to_cities_percent[:self.cities_count - 1] * all_people_left_city_count).astype(int)))
        list_tourist_people = []
        #print(f"Time {self.t}, in city {self.own_ind}, exposed={np.count_nonzero(self.people.exposed[all_people_left_city_inds])} severe={np.count_nonzero(self.people.severe[all_people_left_city_inds])} critical={np.count_nonzero(self.people.critical[all_people_left_city_inds])}")
        for city_ind in range(self.cities_count):
            if city_ind == self.own_ind:
                continue
            tourist_people_inds = people_left_city_to_city_inds[city_ind]
            # едут только те, кто в городе
            tourist_people_inds = tourist_people_inds[self.people.in_city[tourist_people_inds]]
            # конструируем people (туристов), которые должны уйти из нашего города в город с индексом city_ind
            tourist_people = self.select_people_from_inds(tourist_people_inds)
            tourist_people.rest_in_another_city_days = cvu.n_poisson(self.time_relax, tourist_people.pop_size)
            # resulting step
            list_tourist_people.append((city_ind, tourist_people))
            # left people somewhere
            self.people.make_not_existing(tourist_people_inds)
        return list_tourist_people


    def update_rest_days(self):
        inds = self.people.uid[self.people.rest_in_another_city_days >= 0]
        self.people.rest_in_another_city_days[inds] -= 1

    def run(self, do_plot=False, until=None, restore_pars=True, reset_seed=True, verbose=None, barrier=None, lock=None, sims_own_ind=None, shared_tourists=None, shared_back=None, shared_people_inds=None, adjacency_matrix=None, invariant_var=None, tourist_fraction=None):
        '''
        Run the simulation.
        ...
        T = sc.timer()
        is_multiple_cities_mode = shared_tourists is not None
        if is_multiple_cities_mode:
            self.time_relax = 7
            self.pars['own_ind_city'] = sims_own_ind
            self.own_ind = sims_own_ind
            #if self.own_ind == 0:
            #    print()
            self.pars['tourist_ratio'] = tourist_fraction[self.own_ind]
            print(f"My tourist fraction is {self.pars['tourist_ratio']} with {self.own_ind}")
            self.cities_count = len(shared_back) if shared_back is not None else None
            if adjacency_matrix is None:
                self.outflow_ratio_to_cities = np.ones(shape=(self.cities_count), dtype=float) * 0.002
                self.outflow_ratio_to_cities[self.own_ind] = 0.0
            else:
                self.outflow_ratio_to_cities = adjacency_matrix[self.own_ind]
            self.outflow_ratio = np.sum(self.outflow_ratio_to_cities)
            self.outflow_ratio_to_cities_percent = np.zeros(shape=(self.cities_count), dtype=float) if self.outflow_ratio == 0 else self.outflow_ratio_to_cities / self.outflow_ratio
            #print(self.outflow_ratio_to_cities_percent)
            barrier.wait()
        ...
        if is_multiple_cities_mode:
            self.people.contacts.add_layer(tourists_layer=TouristLayer(tourist_count=self.people.tourist_count, number_contact_per_tourist=self['contacts']['c'] * self.pars['tourist_contact_count']))
            self.reset_layer_pars()  # Automatically add layer 'tourists_layer' to the parameters using default values
            self.pars['dynam_layer']['tourists_layer'] = True
            self['iso_factor']['tourists_layer'] = self['iso_factor']['c']
            self['quar_factor']['tourists_layer'] = self['quar_factor']['c']
            self['beta_layer']['tourists_layer'] = self['beta_layer']['c']

            barrier.wait()

        #self.time_in_barrier = 0
        ...
        while self.t < until:
            ...
            if is_multiple_cities_mode:
                ...
                # read shared data
                if self.t > 0:
                    tourists_from = shared_tourists[sims_own_ind]
                    back_people = shared_back[sims_own_ind]
                ...
                barrier.wait()
                #self.time_in_barrier += (time.time() - start_time)
                # ################
                # # read back and tourists from prev step
                self.number_infected_back = 0
                if self.t > 0:
                    for back_people_from_city in back_people:
                        if back_people_from_city is not None:
                            self.add_back(back_people_from_city)
                    #if self.own_ind == 0:
                    #    print()
                    for tourists_from_city in tourists_from:
                        if tourists_from_city is not None:
                            self.add_tourists(tourists_from_city)
                    self.update_rest_days()

                #print("*********")
                #if is_multiple_cities_mode:
                #    print(f"{self.own_ind} number_infected_back = {self.number_infected_back}")
                #print("*********")
                #start_time = time.time()
                barrier.wait()
                #self.time_in_barrier += (time.time() - start_time)
                self.people.inds_new_infections = []
                lock.acquire()
                invariant_var.value += np.count_nonzero(self.people.in_city)                
                lock.release()
                ...
                #self.time_in_barrier += (time.time() - start_time)
                #if self.own_ind == 0:
                #    print(f"Invariant var = {invariant_var.value}, but true_pop_size=1610000")
                #    invariant_var.value = 0
                self.step()


                # write back and tourists to next step
                list_back_people = self.extract_back()
                list_tourists = self.extract_tourists()

                # several times update shared data
                ...
                # Write shared data:
                #
                # assigning back_people
                for (city_ind, back_people) in list_back_people:
                    shared_back[city_ind][sims_own_ind] = back_people
                #
                # assigning tourists people
                for (city_ind, tourists) in list_tourists:
                    shared_tourists[city_ind][sims_own_ind] = tourists

                lock.release()
                #
                # UNLOCK
                ################
                #print(f"Tourist count in {self.own_ind}: {self.people.own_city[(self.people.own_city != self.own_ind) * (self.people.in_city)].size}")
                #print(f"Native people count in {self.own_ind}: {self.people.own_city[(self.people.own_city == self.own_ind) * (self.people.in_city)].size}")
                ...
                self.step()
            self.state_statistics[self.t][self.people.severe] = 1
            self.state_statistics[self.t][self.people.recovered] = 2
            self.state_statistics[self.t][self.people.exposed] = 3
            self.state_statistics[self.t][self.people.dead] = 4
            self.state_statistics[self.t][self.people.critical] = 5
            # fill infected array
            self.infected_array[self.people.severe] = 1
            self.infected_array[self.people.exposed] = 1
            self.infected_array[self.people.dead] = 1
            self.infected_array[self.people.critical] = 1
            self.infected_people_per_day.append(np.count_nonzero(self.infected_array))
            self.infected_array[:] = 0

            #print(self.results['new_infections'].values)
        ...
        self.infected_people_per_day = np.array(self.infected_people_per_day)
        
        #print(f"Time in barrier for {self.own_ind}: {self.time_in_barrier}")
