...
def make_people(sim, prepared_pop=None, popdict=None, die=True, reset=False, recreate=False, verbose=None, **kwargs):
    '''
    ...
    if pop_type == 'synthpops':
        popdict = make_synthpop(sim, prepared_pop=prepared_pop, popdict=popdict, **kwargs)
    ...
    # # add intercity contacts
    # if sim.cities_count is not None:
    #     for city_ind in range(sim.cities_count):
    #         if sim.own_ind == city_ind:
    #             continue
    #         intercity_contacts = ...
...
def make_synthpop(sim=None, prepared_pop=None, popdict=None, layer_mapping=None, community_contacts=None, **kwargs): # pragma: no cover
    '''
    ...
    **Example**::
    ...
    if popdict is None:
        if sim is None: # pragma: no cover
            ...
            raise ValueError(errormsg)
        if prepared_pop is None:
            people = sp.Pop(n=pop_size, rand_seed=sim['rand_seed'], **kwargs).to_people() # Actually generate it
        else:
            print("BEFORE TO PEOPLE")
            people = prepared_pop.to_people()
            print("AFTER TO PEOPLE")
