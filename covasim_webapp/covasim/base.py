    ...
    def export_results(self, for_json=True, filename=None, indent=2, *args, **kwargs):
        ...
        for key,res in self.results.items():
            if isinstance(res, Result):
                if len(res.values.shape) > 1:
                    for var_id in range(res.values.shape[0]):
                        resdict[f"{key}_{var_id}"] = res.values[var_id]
                    continue
...
class Layer(FlexDict):
    ...
    **Examples**::
    ...
    def update(self, people, frac=1.0):
        ...
        pop_size   = people.pop_size # Total number of people
